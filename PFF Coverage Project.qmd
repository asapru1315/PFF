...
coverage_plays <- read_csv("PFF Data/coverage_plays.csv")
coverage_grades <- read_csv("PFF Data/coverage_grades.csv")
grades <- read_csv("PFF Data/grades.csv")
receiving <- read_csv("PFF Data/receiving.csv")
```
...
coverage_analysis_player$weighted_grade <- coverage_analysis_player$average_grade * coverage_analysis_player$percentage
...
coverage_analysis_player$normalized_grade <- (coverage_analysis_player$weighted_grade - min_grade) / (max_grade - min_grade)

significant_players <- coverage_analysis_player %>%
  filter(total_plays > 50)

performance_diff <- significant_players %>%
  pivot_wider(names_from = man_vs_zone, values_from = c(weighted_grade, normalized_grade)) %>%
  filter(!is.na(weighted_grade_man) & !is.na(weighted_grade_zone)) %>%
  mutate(performance_difference = weighted_grade_man - weighted_grade_zone)

top_difference_players <- performance_diff %>%
  arrange(desc(abs(performance_difference))) %>%
  head(10)
